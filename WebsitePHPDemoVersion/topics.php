...
?>
 ...
 <!-- Dropdown Structure -->
   ...
   include('config.php');
        ...
        $query = "SELECT * FROM topics t INNER JOIN consoles c ON t.consoles_id = c.id WHERE t.id = '$id';"; 
    ...
    if ($resultCheck > 0){
        ...
        while ($row = mysqli_fetch_assoc($result)){
            $title = $row['title'];
            $console = $row['manufacturer'] . " " . $row['name'];
            $subject = $row['subject'];
            $date = $row['date'];
            $user = $row['userID'];
...
//            echo $date . "<br>";
//            echo $user . "<br>";
            echo "<h1>" . $title . " " . $console . "</h1>";
            echo "<h2>" . $subject . "</h2>";
            echo "<h3>" . $date . "</h3>";
            echo "<h4>" . $user . "</h4>";
            echo "<a href='replies.php?id=$id'>Replies</a>";
